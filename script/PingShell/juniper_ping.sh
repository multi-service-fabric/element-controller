...
# The syntax for ping command is generated.(The strings separated by "T" are generted)
F_T_LIST=$(echo ${PING_INFO_LIST} | tr '+' ' ');
for F_T in ${F_T_LIST[@]};
do
  FROM=`echo ${F_T} | cut -d_ -f1`
  TO=`echo ${F_T} | cut -d_ -f2`
  PING_COMMAND="ping ${TO} source ${FROM} count ${COUNT} wait ${WAIT} ttl 1"
