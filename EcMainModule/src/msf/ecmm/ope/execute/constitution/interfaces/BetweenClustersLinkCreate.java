...
/**
 * Inter-Cluster Link Generation Class Definition<br>
 * Generate inter-cluster link.
 *
 */
public class BetweenClustersLinkCreate extends Operation {

  /** In case input data check result is NG. */
  private static final String ERROR_CODE_360101 = "360101";

  /** In case the number of pieces of information for process execution is zero. */
  ...
  /** Error has occurred from EM while requesting inter-cluster link generation to EM (error response received). */
   ...
   *          URI key information
   */
  public BetweenClustersLinkCreate(AbstractRestMessage idt, HashMap<String, String> ukm) {
    super(idt, ukm);
    super.setOperationType(OperationType.BetweenClustersLinkCreate);
  }
  ...
  public AbstractResponseMessage execute() {
    ...
    try (DBAccessManager session = new DBAccessManager()) {
      ...
      String ipAddr = inputData.getClusterLink().getAddress();
      int prefix = inputData.getClusterLink().getPrefix();
      PhysicalIfs physicalIfs = DbMapper.toPhysicalIfIpAddrChange(physicalIfsDb, ipAddr, prefix);
      LagIfs lagIfs = DbMapper.toLagIfIpAddrChange(lagIfsDb, ipAddr, prefix);
      BreakoutIfs breakoutIfs = DbMapper.toBreakoutIfIpAddrChange(breakoutIfsDb, ipAddr, prefix);
      session.updateNodeIfIpAddress(physicalIfs, lagIfs, null, breakoutIfs);

      BetweenClustersLinkAddDelete betweenClustersLinkAddDeleteEm = EmMapper.toBetweenClustersLinkCreate(inputData,
          nodesDb);

      AbstractMessage result = EmController.getInstance().request(betweenClustersLinkAddDeleteEm);
