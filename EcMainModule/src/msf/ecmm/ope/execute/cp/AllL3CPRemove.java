...
public class AllL3CPRemove extends Operation {
	...
	public AllL3CPRemove(AbstractRestMessage idt, HashMap<String, String> ukm) {
		super(idt, ukm);
		super.setOperationType(OperationType.AllL3CPRemove);
	}
	...
	public AbstractResponseMessage execute() {
		...
		try (DBAccessManager session = new DBAccessManager()) {
			...
			BulkDeleteL3cp inputData = (BulkDeleteL3cp) getInData();

			List<L3Cps> l3CpsDbList = new ArrayList<L3Cps>();

			for (CpsDeleteCps cps : inputData.getCps()) {

				L3Cps l3CpsDb = session.searchL3Cps(inputData.getSliceId(), cps.getCpId());
				...
				l3CpsDbList.add(l3CpsDb);

			}

			session.deleteL3Cps(l3CpsDbList);

			L3SliceAddDelete l3SliceAddDeleteEm = EmMapper.toL3CPDelete(inputData, l3CpsDbList);
	...
	protected boolean checkInData() {
		...
		try {
			...
			inputData.check(OperationType.AllL3CPRemove);
