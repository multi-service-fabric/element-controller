...
Server_DIR='tftp://$$ECMANAGEMENTADDRESS$$/cumulus/EL2'
export DOWNLOAD_DIR='/home/cumulus'
export Server_DIR='tftp://$$ECMANAGEMENTADDRESS$$/cumulus/EL2'
...
curl ${Server_DIR}/snmpd.conf -o ${DOWNLOAD_DIR}/snmpd.conf
cp -f ${DOWNLOAD_DIR}/snmpd.conf /etc/snmp/snmpd.conf

# Load system configuration file from tftp server
curl ${Server_DIR}/config.txt -o ${DOWNLOAD_DIR}/config.txt
source ${DOWNLOAD_DIR}/config.txt

# Load snmpd config from tftp server
curl ${Server_DIR}/switchd.conf -o ${DOWNLOAD_DIR}/switchd.conf
cp -f ${DOWNLOAD_DIR}/switchd.conf /etc/cumulus/switchd.conf

# Load traffic.conf from tftp server
curl ${Server_DIR}/traffic.conf -o ${DOWNLOAD_DIR}/traffic.conf
cp -f ${DOWNLOAD_DIR}/traffic.conf /etc/cumulus/datapath/traffic.conf

# Load datapath.conf from tftp server
curl ${Server_DIR}/datapath.conf -o ${DOWNLOAD_DIR}/datapath.conf
sudo cp -f ${DOWNLOAD_DIR}/datapath.conf /usr/lib/python2.7/dist-packages/cumulus/__chip_config/bcm/datapath.conf

# Load interface config from tftp server
#curl ${Server_DIR}/interfaces.txt -o ${DOWNLOAD_DIR}/interfaces.txt
...
#curl ${Server_DIR}/ports.conf -o ${DOWNLOAD_DIR}/ports.conf
...
curl ${Server_DIR}/sshd.conf -o ${DOWNLOAD_DIR}/sshd.conf
