...
# # The syntax for the  ping command is generated.(The strings separated with "T" are generated.)
F_T_LIST=$(echo ${PING_INFO_LIST} | tr '+' ' ');
for F_T in ${F_T_LIST[@]};
do
  FROM=`echo ${F_T} | cut -d_ -f1`
  TO=`echo ${F_T} | cut -d_ -f2`
  PING_COMMAND="ping -I ${FROM} -c ${COUNT} -w ${WAIT} -t 1 ${TO}"
